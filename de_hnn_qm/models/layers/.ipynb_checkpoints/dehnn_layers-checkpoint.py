...
class HyperConvLayer(MessagePassing):
    def __init__(self, net_out_channels, out_channels, aggr='add'):
        ...
        self.phi = Seq(Linear(out_channels * 3, out_channels),
                        ReLU(),
        ...
        self.mlp = Seq(Linear(net_out_channels * 2, net_out_channels),
                        ReLU(),
                        Linear(net_out_channels, net_out_channels))
    ...
    def forward(self, x, x_net, edge_index_source_to_net, edge_index_sink_to_net, edge_weight_sink_to_net):
        ...
        h_net_sink = self.psi(self.forward_conv((x, x_net), edge_index_sink_to_net)) + x_net
        ...
        h = self.phi(torch.concat([x, self.conv((h_net, x), torch.flip(edge_index_source_to_net, dims=[0])), 
                                   self.back_conv((h_net, x), torch.flip(edge_index_sink_to_net, dims=[0]))], dim=1)) + x
        return h, h_net
