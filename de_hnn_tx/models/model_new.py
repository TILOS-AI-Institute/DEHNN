...
class GNN_node(torch.nn.Module):
    ...
    def forward(self, data, device):
        # node_features, net_features, edge_index_sink_to_net, edge_weight_sink_to_net, edge_index_source_to_net, edge_weight_net_to_sink, batch, num_vn = data['node'].x.to(device), data['net'].x.to(device), data['node', 'as_a_sink_of', 'net'].edge_index, data['node', 'as_a_sink_of', 'net'].edge_weight, data['node', 'as_a_source_of', 'net'].edge_index.to(device), data['net', 'sink_to', 'node'].edge_weight.to(device), data.batch.to(device), data.num_vn
        node_features, net_features, edge_index_sink_to_net, edge_index_source_to_net, batch, num_vn = data['node'].x.to(device), data['net'].x.to(device), data['node', 'as_a_sink_of', 'net'].edge_index, data['node', 'as_a_source_of', 'net'].edge_index.to(device), data.batch.to(device), data.num_vn
        ...
        # edge_weight_net_to_sink = edge_weight_net_to_sink[edge_mask].to(device)
        
        num_instances = data.num_instances
        h_list = [self.node_encoder(node_features)]
        h_net_list = [self.net_encoder(net_features)]

        #top_batch = torch.zeros(num_vn).long().to(device)
        ...
        for layer in range(self.num_layer):
            if self.vn:
                h_list[layer] = h_list[layer] + virtualnode_embedding[batch] #(virtualnode_embedding + top_embedding[top_batch])[batch]
            
            h_inst, h_net = self.convs[layer](h_list[layer], h_net_list[layer], edge_index_source_to_net, edge_index_sink_to_net, edge_attr_sink_to_net)#, edge_weight_sink_to_net, edge_weight_net_to_sink)
        ...
        node_representation = torch.abs(torch.nn.functional.leaky_relu(self.fc2_node(torch.nn.functional.leaky_relu(self.fc1_node(node_representation), negative_slope = 0.1)), negative_slope = 0.1))
