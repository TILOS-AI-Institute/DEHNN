...
from DEHNN.de_hnn_tx.models.model import GNN_node
...
if restart:
    model = torch.load(f"{model_type}_{num_layer}_{num_dim}_{vn}_{cv}_model.pt")
else:
...
all_train_indices, all_valid_indices, all_test_indices = load_data_indices[:21*25], load_data_indices[21*25:], load_data_indices[21*25:]
...
for epoch in range(100):
    ...
    for data_idx in tqdm(all_train_indices):
        ...
        if vn_representation is None:
            vn_representation = target_map
        ...
        loss = 10*loss_vn + loss_node + 0.001*loss_net
    ...
    if (best_total_val is None) or ((loss_node_all/all_train_idx) < best_total_val):
        ...
        torch.save(model, f"{model_type}_{num_layer}_{num_dim}_{vn}_{cv}_model.pt")
